 ...
 *
 * ShortsStudioLayout is the tabbed studio shell: a header row with the tab
...
export interface ShortsStudioLayoutProps {
  ...
  /** Right side of the header row (search input, filter toggles). */
...
export function ShortsStudioLayout({
  ...
  return (
    <div
      className={cn(
        "flex min-h-dvh flex-col bg-q-background-primary px-4 py-6 text-q-text-primary md:px-8",
        className,
      )}
    >
      <Tabs.Root
  ...
  return (
    ...
    >
      ...
      <footer className="flex flex-col gap-2">
        ...
        >
          {generateLabel}
          {cost ? <span className="text-q-body-sm-regular opacity-80">{cost}</span> : null}
        </Button>
      </footer>
    </section>
  );
