 ...
 * a rounded bordered column — header row (panel title + ghost "Reset"),
...
export interface UpscalerLayoutProps {
  ...
  /** Form panel heading. */
  panelTitle?: string;
  /** Shows a ghost "Reset" button in the panel header when provided. */
  onReset?: () => void;
  submitLabel?: string;
...
export function UpscalerLayout({
  ...
  return (
    <div
      className={cn(
        "min-h-dvh bg-q-background-primary p-4 text-q-text-primary md:p-6",
        className,
      )}
    >
          ...
          >
            ...
            <footer className="sticky bottom-0 border-t border-q-border-subtle bg-q-background-secondary p-4">
              ...
              >
                {submitLabel}
                {cost ? (
                  <span className="text-q-body-sm-regular opacity-80">{cost}</span>
                ) : null}
              </Button>
            </footer>
          </aside>
